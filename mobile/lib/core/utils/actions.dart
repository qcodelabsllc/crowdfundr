...
void openGithubRepo() async {
  final url = Uri.parse('https://github.com/quabynah-bilson/crowdfundr');
  if (await canLaunchUrl(url)) await launchUrl(url);
}

void openTermsOfUse() async {
  final url =
  Uri.parse('https://quabynah-bilson.github.io/crowdfundr/docs/terms_of_use');
  if (await canLaunchUrl(url)) await launchUrl(url);
}

void openPrivacyPolicy() async {
  final url =
  Uri.parse('https://quabynah-bilson.github.io/crowdfundr/docs/privacy');
  if (await canLaunchUrl(url)) await launchUrl(url);
}
